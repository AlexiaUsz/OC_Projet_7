...
def application_train_test(app_data, nan_as_category=False):
    ...
    app_data["INCOME_CREDIT_PERC"] = (
        app_data["AMT_INCOME_TOTAL"] / app_data["AMT_CREDIT"]
    )
    app_data["INCOME_PER_PERSON"] = (
        app_data["AMT_INCOME_TOTAL"] / app_data["CNT_FAM_MEMBERS"]
    )
    app_data["ANNUITY_INCOME_PERC"] = (
...
def bureau_and_balance(
    df_bureau, df_bureau_balance, num_rows=None, nan_as_category=True
):
    """
...
def installments_payments(
    df_installments_payments, num_rows=None, nan_as_category=True
):
    """
...
def credit_card_balance(df_credit_card_balance, num_rows=None, nan_as_category=True):
    ...
    col_num = cc_agg.select_dtypes(
        exclude="O"
    ).columns  # exclude = 'O'; include = 'number'
    # print(col_num)
...
def prediction(df_prep, model_path="../model/model.pkl"):
    ...
    pred_proba = loaded_model.predict_proba(df_prep)[
        :, 1
    ]  # Probabilité associée à la classe 1
